...
import type { DiscordGuild, DiscordChannel, DiscordRole } from "@/lib/discord";
import {
  PLAN_REQUIRED_ERROR,
  SERVER_LIMIT_ERROR,
...
export default function ServerManagementPage() {
  ...
  const id = params?.guildId as string;

  const handleSubscriptionApiError = useCallback(
    (status: number, body: unknown): boolean => {
      ...
      if (message === PLAN_REQUIRED_ERROR) {
        toast.error(
          "Seu plano não está ativo. Abrindo a página de planos para assinar ou renovar.",
        );
        router.replace("/pricing?from=dashboard&reason=inactive-plan");
        return true;
      }

      if (message === SERVER_LIMIT_ERROR) {
        toast.error(
          "Você atingiu o limite de servidores do plano atual. Faça upgrade para continuar.",
        );
        router.replace("/pricing?from=dashboard&reason=server-limit");
        return true;
      }
    ...
    async function loadData() {
      try {
        ...
        if (!guildsResponse.ok) {
          if (handleSubscriptionApiError(guildsResponse.status, guildsData)) {
            return;
          }
          throw new Error(
            getApiErrorMessage(guildsData) ||
        ...
        if (!configResponse.ok) {
          if (handleSubscriptionApiError(configResponse.status, configData)) {
            setServer(null);
            return;
          }
          throw new Error(
            getApiErrorMessage(configData) ||
  ...
  useEffect(() => {
    ...
    async function loadOptions() {
      try {
        ...
        const channelsData = await channelsResponse.json().catch(() => null);

        if (!rolesResponse.ok) {
          if (handleSubscriptionApiError(rolesResponse.status, rolesData)) {
            return;
          }
        }
        ...
        if (!channelsResponse.ok) {
          if (
            handleSubscriptionApiError(channelsResponse.status, channelsData)
          ) {
            return;
          }
        }
  ...
  }, [accessDenied, handleSubscriptionApiError, id, server, serverMissing]);
  ...
  async function handleSave(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        if (handleSubscriptionApiError(response.status, data)) {
          return;
        }
        throw new Error(
          getApiErrorMessage(data) || "Erro ao salvar configurações.",
        );
      }
